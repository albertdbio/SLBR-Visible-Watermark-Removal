...
def save_output(
    inputs, preds, save_dir, img_fn, extra_infos=None, verbose=False, alpha=0.5
):
    outs = []
    ...
    if verbose == True:
        ...
        out_fn = os.path.join(
            save_dir,
            "{}_psnr_{:.2f}_rmsew_{:.2f}_f1_{:.4f}{}".format(
                os.path.splitext(img_fn)[0],
                psnr,
                rmsew,
                f1,
                os.path.splitext(img_fn)[1],
            ),
        )
...
def main(args):
    ...
    with torch.no_grad():
        for i, batches in enumerate(model.val_loader):
            ...
            target_np = (
                target.detach().cpu().numpy()[0].transpose(1, 2, 0) * 255
            ).astype(np.uint8)
            ...
            comp_sets = []
            prime_mask_pred = torch.where(
                out_mask > 0.5, torch.ones_like(out_mask), torch.zeros_like(out_mask)
            ).to(out_mask.device)

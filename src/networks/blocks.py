...
def conv3x3(
    in_channels, out_channels, stride=1, padding=1, bias=True, groups=1, dilation=1
):
    return nn.Conv2d(
        in_channels,
        out_channels,
...
class MBEBlock(nn.Module):
    ...
    def forward(self, from_up, from_down, mask=None):
        # Custom Changes:
        # Stage 3: CoarseDecoder - MBEBlock
...
class SMRBlock(nn.Module):
    def __init__(
        ...
        self.primary_mask = nn.Sequential(*[nn.Conv2d(outs, 1, 1, 1, 0), nn.Sigmoid()])
        self.refine_branch = nn.Sequential(*[nn.Conv2d(outs, 1, 1, 1, 0), nn.Sigmoid()])
        self.self_calibrated = SelfAttentionSimple(
            self.mode,
            outs,
    ...
    def forward(self, input, encoder_outs=None):
        ...
        mask_x, self_calibrated_mask = self.self_calibrated(
            mask_x, mask_x, primary_mask
        )
...
class SelfAttentionSimple(nn.Module):
    def __init__(
        self, mode, in_channel, k_center=1, sim_metric="fc", project_mode="linear"
    ):
        ...
        self.k_weight = nn.Parameter(
            torch.full((1, k_center, 1, 1), fill_value=1, dtype=torch.float32),
            requires_grad=True,
        )
    ...
    def compute_attention(self, query, key, mask, eps=1):  # in: [B, C:128, 64, 64]
        ...
        importance_map = torch.where(
            mask >= self.threshold, torch.ones_like(mask), torch.zeros_like(mask)
        ).to(mask.device)
        s_area = torch.clamp_min(torch.sum(importance_map, dim=[2, 3]), self.min_area)[
            :, 0:1
        ]
        if self.k_center != 2:
            keys = [
                torch.sum(k * importance_map, dim=[2, 3]) / s_area for k in keys
            ]  # b,c * k
        else:
            keys = [
        ...
        f_key = [
            k.reshape(b, c, 1, 1).repeat(1, 1, f_query.size(2), f_query.size(3))
            for k in keys
        ]
        attention_scores = []
        for k in f_key:
        ...
        if self.k_center == 2:
            ...
            v_bg = torch.sum(v[:, c:] * (1 - importance_map), dim=[2, 3]) / (
                v.shape[2] * v.shape[3] - s_area + eps
            )
        ...
        attn = (
            torch.bmm(s.reshape(b, h * w, self.k_center), v)
            .reshape(b, h, w, c)
            .permute(0, 3, 1, 2)
        )
...
class ResDownNew(nn.Module):
    def __init__(self, in_size, out_size, pooling=True, use_att=False, dilation=False):
        ...
        self.model = DownConv(
            ...
            norm=nn.InstanceNorm2d,
            act=F.leaky_relu,
            dilations=[1, 2, 5] if dilation else [],
        )
...
class ResUpNew(nn.Module):
    def __init__(self, in_size, out_size, use_att=False):
        ...
        self.model = UpConv(
            in_size, out_size, 3, use_att=use_att, norm=nn.InstanceNorm2d
        )
